...
public:
    ...
    struct instance_halls_of_stone_InstanceMapScript : public InstanceScript
    {
		...
		bool CheckAchievementCriteriaMeet(uint32 criteria_id, Player const* /*source*/, Unit const* /*target*/, uint32 /*miscvalue1*/)
        {
            switch(criteria_id)
            {
               case 7143: // Maiden of Grief
                   if(Difficulty(instance->GetSpawnMode()) == DUNGEON_DIFFICULTY_HEROIC && (maxGriefDeadTime - mGDT) <= 60)
                      return true;
                   return false;
               case 7593: // Sjonnir the Ironshaper
                   if(Difficulty(instance->GetSpawnMode()) == DUNGEON_DIFFICULTY_HEROIC)
                      return true;
                   return false;
            }
            return false;
